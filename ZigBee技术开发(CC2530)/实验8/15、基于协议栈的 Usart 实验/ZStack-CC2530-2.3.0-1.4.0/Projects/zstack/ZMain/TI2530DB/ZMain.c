...
{
  // Turn off interrupts
  osal_int_disable( INTS_ALL );     //关所有中断---by Cavani

  // Initialization for board related stuff such as LEDs
  HAL_BOARD_INIT();     //初始化板载外设  
  ...
  InitBoard( OB_READY );    //初始化板载外设

  // Display information about this device
  zmain_dev_info();     //显示相关信息
...
#endif
  // No Return from here
  osal_start_system();  //执行操作系统，进去后不会返回
